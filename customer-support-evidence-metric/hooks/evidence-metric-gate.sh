#!/usr/bin/env bash
# customer-support-evidence-metric gate: requires at least one evidence
# metric (CSAT/FCR/SLA-adherence) cited on any handbook/record write.
# Sources core's gate-lib.sh (issue-72 gate-house standard),
# reference-adopt not vendor (issue-13).

. "${CLAUDE_PLUGIN_ROOT_CORE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../core" && pwd -P)}/hooks/lib/gate-lib.sh" \
  || { echo "evidence-metric-gate.sh: cannot source gate-lib.sh" >&2; exit 2; }
gate_trap_fail_closed
set -uo pipefail
gate_kill_switch_active "${CUSTOMER_SUPPORT_EVIDENCE_METRIC_GATE_OFF:-}" || { trap - EXIT; exit 0; }
...
GATE_NAME = "customer-support-evidence-metric"
...
if not re.search(r'csat|fcr|first contact resolution|sla-adherence|sla adherence', content, re.IGNORECASE):
    deny("write is missing required element(s): evidence-metric. "
         f"Per {DOC_REF}, every phase-2 deliverable/record write must carry: at least "
         "one evidence metric (CSAT, FCR/First Contact Resolution, or SLA-adherence) "
         "cited.")
